...
export default function NewsDetailPage({ params }: IPageProps) {
	const { slug } = params || {};
	...
	const news = allNews.find((item) => item.id === Number(slug));

	// Get related news (same category, excluding current)
	const relatedNews = allNews
		.filter(
			(item) => item.category === news?.category && item.id !== Number(slug)
		)
		.slice(0, 3);
